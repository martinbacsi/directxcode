/*
Description:  This demo show you how to Create spot light in Direct3D 10
Date: 2013-07-25
Author: zdd
*/
...
ID3D10EffectMatrixVariable* g_pWorldViewProjVariable= NULL;
ID3D10EffectVariable*		g_pEyePosVariable		= NULL;
ID3D10EffectVariable*		g_pLightVariable		= NULL;
...
D3DXMATRIX g_mView;
D3DXMATRIX g_mProj;
D3DXMATRIX g_mWorldViewPorj;

D3DXVECTOR3 g_EyePos;	// Eye position used in lighting
...
VOID InitLight()
{
	// Point light does not have direction
    g_Light.dir = D3DXVECTOR3(1.0f, 1.0f, -1.0f);
	//g_Light.pos = D3DXVECTOR3(50.0f, 50.0f, -50.0f);
	g_Light.range = 100.0f;
	g_Light.att.x = 1.0f;
	g_Light.att.y = 0.0f;
	g_Light.att.z = 0.0f;
	g_Light.spotPow = 64.0f;
	g_Light.ambient = D3DXCOLOR(0.2f, 0.2f, 0.0f, 1.0f);
	g_Light.diffuse = D3DXCOLOR(1.0f, 1.0f, 0.0f, 1.0f);
	//g_Light.specular = D3DXCOLOR(1.0f, 1.0f, 0.0f, 1.0f);
}
...
VOID SetupMatrix(float timeDelta)
{
	...
	lightPos.x = 50.0f * cosf(timeElapsed);
	lightPos.y = 50.0f;
	lightPos.z = 50.0f * sinf(timeElapsed);

	g_Light.pos = lightPos;
	...
	g_pWorldViewProjVariable->SetMatrix((float*)&g_mWorldViewPorj);
}

VOID SetupLight()
{
	g_pEyePosVariable->SetRawValue(&g_EyePos, 0, sizeof(D3DXVECTOR3));
	g_pLightVariable->SetRawValue(&g_Light, 0, sizeof(Light));
}
...
INT WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR szCmdLine, int iCmdShow)
{
	...
	winClass.lpszClassName = L"Spot Light";
