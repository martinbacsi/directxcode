...
IDirect3DSurface9*		g_pSurfaceTopleft	= NULL ;
IDirect3DSurface9*		g_pSurfaceTopright	= NULL ;
...
VOID Cleanup()
{
	...
	// Release top-left render target surface
	SAFE_RELEASE(g_pSurfaceTopleft) ;

	// Release top-right render target surface
	SAFE_RELEASE(g_pSurfaceTopright) ;
}
...
VOID Render()
{
	...
	// Set render target
	hr = g_pd3dDevice->SetRenderTarget(1, g_pSurfaceTopleft) ;
	if (FAILED(hr))
	{
		MessageBox(NULL, "Set Top-left render target failed!", "Error", 0) ;
	}
	// Clear the back-buffer to a RED color

	hr = g_pd3dDevice->SetRenderTarget(2, g_pSurfaceTopright) ;
	if (FAILED(hr))
	{
		MessageBox(NULL, "Set Top-right render target failed!", "Error", 0) ;
	}
