...
void Init()
{
	...
	g_Terrain->initialize(g_pd3dDevice);

	// Init view and proj matrix
	g_Camera = new Camera();
	...
	g_Camera->SetViewParams(eyePt, lookCenter, upVec);
...
void Render(float timeDelta)
{
	...
	if( SUCCEEDED( g_pd3dDevice->BeginScene() ) )
	{
		...
		// Render Terrain
		g_Terrain->render(g_pd3dDevice, g_Camera);

		int drawCount = g_Terrain->getDrawCount();

		char buffer[10] ;
		_itoa_s( drawCount, buffer, sizeof(buffer), 10);
		POINT point = { 10, 10 } ;
