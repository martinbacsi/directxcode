...
LetterObject::~LetterObject(void)
{
...
bool LetterObject::isLive() const
{
	return isLive_;
}

void LetterObject::setLiveState(bool liveFlag)
{
	isLive_ = liveFlag;
}

bool LetterObject::getVisible() const
{
	return isVisible_;
}

void LetterObject::setVisible(bool visibleFlag)
{
	isVisible_ = visibleFlag;
}

wchar_t LetterObject::getLetter() const
{
	return letter_;
}

void LetterObject::setLetter(wchar_t letter)
{
	letter_ = letter;
}

D2D1_COLOR_F LetterObject::getFillColor() const
{
	return fillColor_;
}

void LetterObject::setFillColor(D2D1_COLOR_F color)
{
	fillColor_ = color;
	pFillBrush->SetColor(fillColor_);
}

D2D1_COLOR_F LetterObject::getOutlineColor() const
{
	return outlineColor_;
}

void LetterObject::setOutlineColor(D2D1_COLOR_F color)
{
	outlineColor_ = color;
	pOutlineBrush->SetColor(outlineColor_);
}

float LetterObject::getSpeedFactor() const
{
	return speedFactor_;
}

void LetterObject::setSpeedFactor(float speedFactor)
{
	speedFactor_ = speedFactor;
}

void LetterObject::setBoundaryColor(D2D1_COLOR_F& color)
{
	boundaryBrush_->SetColor(color);
}

void LetterObject::setBoundaryBackgroundColor(D2D1_COLOR_F& color)
{
	boundaryBackgroundBrush_->SetColor(color);
}

float LetterObject::getoutlineWidth() const
{
	return outlineWidth_;
}

void LetterObject::setOutlineWidth(float width)
{
	outlineWidth_ = width;
}

int LetterObject::getSize() const
{
	return fontSize_;
}

void LetterObject::getBound(D2D1_RECT_F* rect) const
{
	pTransformedGeometry_->GetBounds(D2D1::Matrix3x2F::Identity(), rect);
}

D2D1_POINT_2F LetterObject::getPosition() const
{
	D2D1_POINT_2F pos;
...
void LetterObject::setPosition(D2D1_POINT_2F pos)
{
	translate(pos.x, pos.y);
}

void LetterObject::setPosition(float x, float y)
{
	translate(x, y);
}

D2D1_POINT_2F LetterObject::getVelocity() const
{
	return velocity_;
}

void LetterObject::setVelocity(D2D1_POINT_2F velocity)
{
	velocity_ = velocity;
}

void LetterObject::setVelocity(float x, float y)
{
	velocity_.x = x;
	velocity_.y = y;
}

D2D1_RECT_F	LetterObject::computeBoundary() const
{
	D2D1_RECT_F rect;
	pTransformedGeometry_->GetBounds(D2D1::Matrix3x2F::Identity(), &rect);
	return rect;
...
void LetterObject::setTransform(D2D1_MATRIX_3X2_F& matrix)
{
...
ID2D1TransformedGeometry* LetterObject::getTransformedGeometry() const
{
